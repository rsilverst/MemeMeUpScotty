...
| **PR 16** | E14 | 2026-06-04 | Save/share as WebP lossless. Discussed and rejected the settings-menu route — one toggle isn't worth a new screen on a personal app — and picked a single sensible default instead: `Bitmap.CompressFormat.WEBP_LOSSLESS` on API 30+, falling back to PNG on API 26-29 (the deprecated `WEBP` constant is lossy at every quality level, so PNG is the right pre-30 fallback to keep the caption outlines crisp). A single file-scope `outputFormat` record (compress format + extension + MIME type) feeds both `saveBitmapToGallery` (MediaStore display name, MIME column, compress call) and `shareBitmap` (cache filename, compress call, share intent MIME type). WebP lossless is ~25-30% smaller than PNG for typical meme content with no visible degradation on the caption strokes. Build green, 23/23 tests still passing (no test changes — the format is an opaque sink). |
| **PR 15** | E5 | 2026-06-04 | Session-long generation history carousel. **VM**: new `generationHistory: StateFlow<List<File>>`. Both `setLoadedImage()` (gallery picks) and successful `generateImage()` now append to history (most-recent at index 0); the prior single-slot `lastGeneratedFile` field is gone along with all the previous-file deletion logic (cold-start sweep at `MainActivity.onCreate` handles cleanup at next launch, so nothing is deleted during the live session). New public `selectFromHistory(file)` flips the active state without touching the history list. The `appendToHistory` helper dedupes by file identity defensively. `onCleared` cleanup also dropped — the cold-start sweep is authoritative. **UI**: new `ui/HistoryStrip.kt` — `LazyRow` of 52dp square Coil-backed `AsyncImage` thumbs in 10dp-rounded `Surface`s, 8dp horizontal arrangement. Selected thumb gets a 2dp `Plasma500` border, unselected uses 1dp `Space500`. Header: `"THIS SESSION · N"` in `labelMedium`. Strip hides itself entirely when `history.size < 2` — a strip-of-one is just noise. A right-edge `Brush.horizontalGradient(Transparent → Space900)` overlay advertises "more available, scroll for it" without the user having to touch the strip first. **Wiring**: `MemeScreen` collects `generationHistory`, derives `activeFile` from the current `GenerationState.Success`, threads both plus `onSelectFromHistory = viewModel::selectFromHistory` through `MemeContent` → `CompactLayout` / `ExpandedLayout`. Compact: strip sits between canvas and Save/Share. Expanded: strip sits under the HUD in the left column. **Strings**: `history_label` = "This session", `history_thumb_content_description` = "Recent generation". **Tests**: rewrote `setLoadedImage_deletesPreviouslyTrackedFile` → `setLoadedImage_appendsToHistoryAtHead_andDoesNotDeletePrevious` (asserts both files survive + most-recent at index 0); rewrote `successfulGeneration_deletesPreviouslyTrackedFile` → `successfulGeneration_prependsToHistory_andKeepsPriorEntries` (asserts prior history entries survive + new entry at head); new `selectFromHistory_flipsActiveButDoesNotMutateHistory`. 23/23 unit tests passing (9 VM + 14 repo). |
...
### E5. No generation history [P2] ✅ **DONE (PR 15, 2026-06-04)**
...
**Implementation note:** went broader than "last N" — the carousel is **unbounded for the live session**, cleared on app close (matches the existing `MainActivity.onCreate` cache sweep that already wipes `generated_meme_*` / `gallery_meme_*` / `shared_meme_*` at next launch). User decision after seeing the mockup. Most-recent at index 0; the strip sits between the canvas and Save/Share (compact) and under the HUD in the left column (expanded). Hidden until 2+ entries. Tap a thumb → `selectFromHistory(file)` flips the active canvas image; captions / offsets / sizes / styles persist (they live in `MemeCanvas` state, not tied to the image). Gallery-picked images via `setLoadedImage` also append to history. File lifecycle: nothing is deleted during the live session — the cold-start sweep handles cleanup. Visual: right-edge horizontal-gradient fade (`Brush.horizontalGradient(transparent → Space900)`) advertises "more available, scroll for it" before the user touches the strip. Two VM tests rewritten (the prior deletion-pinning tests no longer reflect behavior), one new test for `selectFromHistory` not mutating history.
...
### E14. No share/save format choice [P3] ✅ **DONE (PR 16, 2026-06-04)** — *settings menu skipped, single sensible default chosen*
Saves PNG only. JPG would be smaller. WebP smaller still. Power users would appreciate a setting.

**Implementation note:** discussed and rejected the settings-menu route — one toggle isn't worth a new screen on a personal app. Instead, picked a single sensible default: **WebP lossless** on API 30+, falls back to PNG on API 26-29 where `WEBP_LOSSLESS` doesn't exist (the deprecated `WEBP` constant is lossy even at quality=100, so PNG is the right fallback to preserve the high-contrast caption strokes). Both `saveBitmapToGallery` and `shareBitmap` now consume a single `outputFormat` triple (compress format + extension + MIME type) defined once at file scope. WebP lossless is ~25-30% smaller than PNG for typical meme content with no visible degradation on the caption outlines.
...
| E5 | Product | P2 | ✅ No generation history *(PR 15, unbounded session carousel with fade affordance under canvas)* |
...
| E13 | Legal | P0 | (dup A6) |
| E14 | Product | P3 | ✅ No format choice on save *(PR 16, WebP lossless on API 30+, PNG fallback on 26-29; no settings screen)* |
...
- **PR 15 — Generation history carousel:** E5 (unbounded session carousel, hidden under <2 entries, right-edge fade affordance, in-memory only). ✅ **landed 2026-06-04.**
- **PR 16 — WebP lossless output:** E14 without a settings menu. WebP lossless on API 30+, PNG fallback on 26-29. ✅ **landed 2026-06-04.**
